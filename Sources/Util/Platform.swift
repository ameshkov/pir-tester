...
public struct Platform: Equatable, Hashable, Sendable {
    ...
    public init?(userAgent: String) {
        // matches ... iOS/18.0 ...
        let iOSRegex: Regex = #/^.*iOS/(\d+)\.(\d+)\.?(\d)?.*/#
        // matches ... Macintosh; OS X 15.0.1 ...
        let macOSRegex: Regex = #/^.*Macintosh; OS X (\d+)\.(\d+)\.?(\d)?.*/#

        func parseOsVersion(
            // swiftlint:disable:next large_tuple
            from match: Regex<(Substring, Substring, Substring, Substring?)>.Match
        ) -> OsVersion? {
