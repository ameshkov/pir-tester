...
extension RandomNumberGenerator {
    ...
    mutating func fill(_ bufferPointer: UnsafeMutableRawBufferPointer) {
        ...
        for i in stride(
            from: bufferPointer.startIndex,
            through: bufferPointer.endIndex &- size,
            by: size
        ) {
            var random = next()
            withUnsafeBytes(of: &random) { randomBufferPointer in
                let rebased = UnsafeMutableRawBufferPointer(
                    rebasing: bufferPointer[i..<(i &+ size)]
                )
        ...
        if !remainingSlice.isEmpty {
            var random = next()
            withUnsafeBytes(of: &random) { randomBufferPointer in
                for (sliceIndex, randomIndex) in zip(
                    remainingSlice.indices,
                    randomBufferPointer.indices
                ) {
