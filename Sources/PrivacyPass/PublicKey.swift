...
public struct PublicKey: Sendable {
    private static var rsassaPSSParams: RSASSAPSSParams {
        ...
        try! RSASSAPSSParams(
            hashAlgorithm: .init(algorithm: .HashAlgorithmIdentifier.sha384),
            maskGenAlgorithm: .init(
                algorithm: .MaskGenAlgorithmIdentifier.mgf1,
                parameters: ASN1Any(
                    erasing: AlgorithmIdentifier(
                        algorithm: .HashAlgorithmIdentifier
                            .sha384
                    )
                )
            ),
            saltLength: [UInt8(PrivacyPass.TokenTypeBlindRSASaltLength)],
            trailerField: [1][...]
        )
    }
...
private struct AlgorithmIdentifier: DERImplicitlyTaggable, Equatable {
    ...
    init(
        ...
        parameters: ASN1Any? = nil,
        explicitlyEncodeNil: Bool = false
    ) {
