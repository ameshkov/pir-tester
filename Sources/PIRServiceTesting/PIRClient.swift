...
public struct PIRClient<PIRClient: IndexPirClient> {
    ...
    public mutating func request(
    ...
    ) async throws -> [KeywordValuePair.Value?] {
        ...
        guard let storedSecretKey = secretKeys[config.evaluationKeyConfigHash] else {
            ...
            throw PIRClientError.missingSecretKey(
                evaluationKeyConfigHash: Array(config.evaluationKeyConfigHash)
            )
        }
        ...
        let pirRequests: [Apple_SwiftHomomorphicEncryption_Api_Pir_V1_PIRRequest] = try keywords.map
        { keyword in
    ...
    public mutating func symmetricPirRequest(
        ...
        guard let configuration = configCache[usecase] else {
            if allowKeyRotation {
                try await rotateKey(for: usecase)
                return try await symmetricPirRequest(
                    keywords: keywords,
                    usecase: usecase,
                    allowKeyRotation: false
                )
        ...
        let parsedOprfResponses: [OprfClient.ParsedOprfOutput] = try zip(
            responses.responses,
            oprfQueryContexts
        )
        ...
        return try zip(keywordPirResponses, parsedOprfResponses).map {
            keywordPirResponse,
            oprfResponse in
    ...
    private func keywordPIRClient(
        ...
        config: Apple_SwiftHomomorphicEncryption_Api_Pir_V1_PIRConfig,
        context: Context<PIRClient.Scheme>
    ) throws -> KeywordPirClient<PIRClient> {
