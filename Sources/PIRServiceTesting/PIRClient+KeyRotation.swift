...
extension PIRClient {
    ...
    {
        let configRequest = Apple_SwiftHomomorphicEncryption_Api_Pir_V1_ConfigRequest.with {
            configRequest in
            configRequest.usecases = [usecase]
            configRequest.existingConfigIds = [configCache[usecase]?.configurationId ?? Data()]
        }

        let configResponse: Apple_SwiftHomomorphicEncryption_Api_Pir_V1_ConfigResponse =
            try await post(
                path: "/config",
                body: configRequest
            )
        ...
        configCache[usecase] = Configuration(
            config: config.pirConfig,
    ...
    mutating func rotateKey(for usecase: String) async throws {
        ...
        let context = try Context<PIRClient.Scheme>(
            encryptionParameters: config.encryptionParameters.native()
        )
        ...
        let evaluationKey = try context.generateEvaluationKey(
            config: keyStatus.keyConfig.native(),
            using: secretKey
        )
        secretKeys[config.evaluationKeyConfigHash] = storedSecretKey

        let evaluationKeyWithMetadata =
            Apple_SwiftHomomorphicEncryption_Api_Shared_V1_EvaluationKey.with { evalKey in
    ...
    mutating func uploadKey(
        _ key: Apple_SwiftHomomorphicEncryption_Api_Shared_V1_EvaluationKey
    ) async throws {
        let keys = Apple_SwiftHomomorphicEncryption_Api_Shared_V1_EvaluationKeys.with {
            keyRequest in
            keyRequest.keys = [key]
        }
